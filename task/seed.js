...
const main = async () => {
	...
	const reviewsList1 = ["Totally Agree", " I feel the same way", "Well, make sense.", "Nod", "It reminds me of my own experiences.",
		"Nod,nod", "LOL", "Tomato, potato.", "I have no idea.", "You think?", "Not agreen.", " Can not agree.", "I don't think so...", "why?",
		"Well, not my type but make some sense."]
		...
		try {
			...
			while (round >= 0) {
				...
				await reviews.addReview(reviewsList1[reviewIndex],userIdList[userIndex],answerIdList[answerIndex]);
				round--;
			}
		} catch (error) {
			console.log(error);
